...
def img_qr_func(img_file, img_size_x, img_size_y, img_caption):
    screenvar = displayio.Group()
    ...
    bg_black[0] = color_black
    bg_sprite = displayio.TileGrid(bg_gif,pixel_shader=bg_black,x=0,y=0)
    screenvar.append(bg_sprite)

    # Add image
    x_off_center_calc = int((DISPLAY_WIDTH/2) - (img_size_x/2))
    f = open(img_file, "rb")
    pic = displayio.OnDiskBitmap(f)
    # Create a Tilegrid with the bitmap and put in the displayio group
    # CircuitPython 6 & 7 compatible
    t = displayio.TileGrid(pic, pixel_shader=getattr(pic, "pixel_shader", displayio.ColorConverter()), x=x_off_center_calc, y=0)
    ...
    # Add caption
    # add text - img caption
    if len(img_caption) > 0:
        img_caption_scale = 2 if len(img_caption) < 26 else 1
        img_caption_size = len(img_caption)
        img_caption_xpixels = img_caption_scale * img_caption_size * FONT_X_SIZE    # total pixel width of the string
        x_off_center_calc = int((DISPLAY_WIDTH/2) - (img_caption_xpixels/2))
        y_off_calc = int(img_size_y+(FONT_Y_SIZE*img_caption_scale))
        text_group_img_caption = displayio.Group(scale=img_caption_scale, x=x_off_center_calc, y=y_off_calc)
        text_area_img_caption = label.Label(font, text=img_caption, color=color)
        text_group_img_caption.append(text_area_img_caption) # subgroup for text scaling
        screenvar.append(text_group_img_caption)
    img_caption
